...
function LocationCard({ info }: CardProps) {
    ...
    const [currentLocation, setCurrentLocation] = useState<LocationBlock | null>(null);
...
useEffect(() => {
    ...
    if (data && info?.id) {
        ...
        const updated = locations.find((l) => l.id === info.id);
        setCurrentLocation(updated || info);
    } else {
        ...
        <>
            <li key={info.id} className={styles.wrapperCard}>
                ...
                <div className={styles.wrapperInfoCard}>
                    <p className={styles.textPlace}>{currentLocation?.placeType  || info.placeType}</p>
                    <div>
                        {[1, 2, 3, 4, 5].map((star) => (
                            <Star
                                key={star}
                    ...
                    <p className={styles.textNameLocation}>{currentLocation?.name  || info.name}</p>
                </div>
                <button
