...
function LocationCard({ info }: CardProps) {
  ...
  useEffect(() => {
    ...
    if (data && info?.id) {
      ...
      const updated = locations.find((l) => l.id === info.id) || null;

      // updated може бути undefined, тому || null
      setCurrentLocation(updated ?? null);
    } else {
      // тут info точно не null, але тип LocationBlock може відрізнятися,
      // якщо треба, можеш явно зкастити: info as unknown as LocationBlock
      setCurrentLocation(null);
    }
  }, [info]);

  const displayPlaceType = currentLocation?.placeType ?? info.placeType ?? "";
  const displayName = currentLocation?.name ?? info.name ?? "";
  ...
  return (
    <li className={styles.wrapperCard}>
      ...
      <div className={styles.wrapperInfoCard}>
        <p className={styles.textPlace}>{displayPlaceType}</p>
        ...
        <p className={styles.textNameLocation}>{displayName}</p>
      </div>
